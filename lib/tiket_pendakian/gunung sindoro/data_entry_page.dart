...
class DataEntryPagesindoro extends StatefulWidget {
  final int jumlahRombongan;

  const DataEntryPagesindoro({super.key, required this.jumlahRombongan,});

  @override
  _DataEntryPageStatesindoro createState() => _DataEntryPageStatesindoro();
}

class _DataEntryPageStatesindoro extends State<DataEntryPagesindoro> {
  ...
  Future<void> _navigateToFormPage(int index) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DataFormPagesindoro(initialData: _passengerData[index]),
      ),
    );
...
void _navigateToPilihKetuaPage() {
  if (_isDataFilled()) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PilihKetuaPagesindoro(teamMembers: teamMembersList),
      ),
    );
  } else {
