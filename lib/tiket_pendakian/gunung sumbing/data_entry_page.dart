...
class DataEntryPagesumbing extends StatefulWidget {
  final int jumlahRombongan;

  const DataEntryPagesumbing({super.key, required this.jumlahRombongan,});

  @override
  _DataEntryPageStatesumbing createState() => _DataEntryPageStatesumbing();
}

class _DataEntryPageStatesumbing extends State<DataEntryPagesumbing> {
  ...
  Future<void> _navigateToFormPage(int index) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DataFormPagesumbing(initialData: _passengerData[index]),
      ),
    );
...
void _navigateToPilihKetuaPage() {
  if (_isDataFilled()) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PilihKetuaPagesumbing(teamMembers: teamMembersList),
      ),
    );
  } else {
